...
f, ax = plt.subplots()
ax2 = ax.twinx()
ax.plot(range(nturbs), uwake(a))
ax2.plot(range(nturbs), 4 * a * (1 - a ) ** 2 * .5 * rho * uwake(a)**3 * D ** 2 / 4 * np.pi / 1e3, ls='--')
plt.xlabel('Turbine')
ax.set_ylabel('Speed')
...
f, ax = plt.subplots()
ax2 = ax.twinx()
ax.plot(range(nturbs), uwake(astar), c='r')
ax2.plot(range(nturbs), 4 * a * (1 - a) ** 2 * .5 * uwake(astar)**3 * rho)
