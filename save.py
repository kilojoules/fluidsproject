...
def fitness(x):
    ...
    return -1 * powa + 1e2 * spacing(x) + bounds(x)
def spacing(x):
    xp = x[:nturbs]
    yp = x[nturbs:]
    penalty = 0.
...
for i in range(nturbs):
    ...
    for j in range(i + 1, nturbs):
        exec("confs.append(lambda x: -1000 * np.sqrt((x[:nturbs][{}] - x[:nturbs][{}])**2 + (x[nturbs:][{}] - x[nturbs:][{}])**2) if (np.sqrt((x[:nturbs][{}] - x[:nturbs][{}])**2 + (x[nturbs:][{}] - x[nturbs:][{}])**2) < 2 * D) else 0)".format(i, j , i, j, i, j, i, j))
...
c = plt.scatter(txopt, tyopt, c = power(uwake(tx,ty, tyopt, tyopt)) / 1e6, cmap=plt.cm.coolwarm)
plt.colorbar(c)
plt.title('Power %f'%(-1 * fitness(xopt)))
plt.title("Power = %f MW" % ( np.sum(power(uwake(tx,ty, tx, ty)))/1e6))
